...
private:
    ...
    string collectSubstring(Node* node, int start, int length) const;
...
public:
    struct Action {
        ...
        int cursorAfter;
        string data;
        Action(const string& name, int before, int after, const string& d)
            : actionName(name), cursorBefore(before), cursorAfter(after), data(d) {}

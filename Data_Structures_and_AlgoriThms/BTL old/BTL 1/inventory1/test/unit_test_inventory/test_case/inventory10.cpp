...
bool UNIT_TEST_inventory::inventory10()
{
    string name = "inventory10";
...
Index is invalid!\n";
    ...
    try
    {
        output<< inventory.getProductAttributes(2)<<endl;
    }
    ...
    try
    {
        output<< inventory.getProductAttributes(4)<<endl;
    }
    catch(const std::exception& e)
    {
        output << e.what() << '\n';
    ...
    try
    {
        output<< inventory.getProductAttributes(-1)<<endl;
    }
    catch(const std::exception& e)
    {
        output << e.what() << '\n';

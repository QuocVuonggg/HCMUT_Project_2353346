...
void Heap<T>:: heapsort(XArrayList<T>& arrayList){
  clear();
    ...
    // Step 1: Create a working copy to build the heap
    XArrayList<T> temp;
    for (int i = 0; i < n; ++i) {
        T val = arrayList.removeAt(0);
        temp.add(val);
        arrayList.add(val);
    }
    ...
    for (int i = 0; i < n; ++i) {
        push(temp.removeAt(0));
    }

    // Step 3: Sort by replacing elements from left to right
    XArrayList<T> sorted;
    for (int i = 0; i < n; ++i) {
        T minVal = pop();
        
        T oldVal = arrayList.removeAt(i);
        arrayList.add(oldVal);
        
        for (int j = arrayList.size() - 1; j > i; j--) {
            T t = arrayList.removeAt(j-1);
            arrayList.add(t);
        }

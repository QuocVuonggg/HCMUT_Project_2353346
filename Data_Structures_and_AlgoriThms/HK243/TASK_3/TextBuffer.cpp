...
void TextBuffer::moveCursorLeft() {
    ...
    undoStack.insertAtTail(new Action("move", oPos, 'L'));
}
...
void TextBuffer::moveCursorRight() {
    ...
    undoStack.insertAtTail(new Action("move", oPos, 'R'));
}
...
void TextBuffer::moveCursorTo(int index) {
    ...
    undoStack.insertAtTail(new Action("move", oPos, 'J'));
}
...
void TextBuffer::undo() {
    ...
    if(act->actionName == "insert") {
        buffer.deleteAt(act->actcur);
        cursorPos = act->actcur;
    } else if(act->actionName == "delete") {
        buffer.insertAt(act->actcur, act->character);
        cursorPos = act->actcur + 1;
...
void TextBuffer::HistoryManager::printHistory() const {
    ...
    for(int i=0;i<history.size();i++){
        ...
        ss<<"("<<act->actionName<<", "<<act->cursorPos<<", ";
        if(act->character != '\0') {
            ss << act->character<<")";
        }
        if(i<history.size()-1){

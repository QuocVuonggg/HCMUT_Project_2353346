/*
 * PROJECT: GAIT ANALYSIS - V13.5 (SILENT SERIAL & SMART SD)
 * Change:
 * - SERIAL: Xóa hết in ra Serial Monitor.
 * - SD CARD: 
 * + Keep-Open: Mở file khi Start, giữ nguyên khi Pause, chỉ đóng khi Stop.
 * + Pause Logic: Flush dữ liệu để an toàn, tính bù thời gian (Time) để nối tiếp khi Resume.
 * + Format: .csv (Time,FSR,Am,Gyro,Phase,Step), 20ms, Không Header.
 * - BLUETOOTH: Giữ nguyên (100ms).
 */
...
File dataFile; // Biến file toàn cục
...
// Biến tính toán bước chân
float maxHorizontalAccel = 0;
float maxSwingGyro = 0; 

// --- BIẾN QUẢN LÝ THỜI GIAN PAUSE ---
...
// Biến nút nhấn
int buttonState = HIGH; int lastButtonState = HIGH;
...
void runGaitLogic() {
  ...
  // --- GHI THẺ SD (20ms) - KEEP OPEN MODE ---
  static unsigned long lastSDPrint = 0;
  if (now - lastSDPrint >= 20) {
    lastSDPrint = now;
    if (dataFile) { // Kiểm tra nếu file còn đang mở
      ...
      // Tự động Flush mỗi ~1 giây (50 mẫu) để tránh mất dữ liệu nếu lỏng pin
      sampleCounter++;
      if (sampleCounter >= 50) {
        dataFile.flush();
        sampleCounter = 0;
...
void handleShortPress() {
  switch (sysState) {
    case OFF:
      ...
      // Mở file và GIỮ NGUYÊN (Keep Open)
    ...
    case RUNNING:
      ...
      // KHÔNG ĐÓNG FILE. Chỉ flush để lưu dữ liệu an toàn.
      // Dữ liệu thời gian sẽ tự động ngừng tăng nhờ totalPausedTime.
    ...
    case PAUSED:
      // --- RESUME ---
      sysState = RUNNING;
      // Cộng dồn thời gian đã nghỉ vào biến tổng
      totalPausedTime += (millis() - pauseStartTime);
      lastLoopTime = millis();
      
      // FILE VẪN ĐANG MỞ, KHÔNG CẦN LÀM GÌ THÊM

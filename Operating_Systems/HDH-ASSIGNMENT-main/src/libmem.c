...
//  struct sc_regs regs;
 ...
 addr_t tgtfpn;
    ...
    // try to get free frame from RAM first
    if (MEMPHY_get_freefp(caller->krnl->mram, &tgtfpn) == 0){
      // got free frame in RAM
    ...
    //pte_set_fpn(...);
    // get frame RAM from the victim page
    tgtfpn = vicfpn; // re-use the freed frame

    if (pte & PAGING_PTE_SWAPPED_MASK){
      addr_t old_swpfpn = PAGING_SWP(pte);
      // copy data from swap to RAM

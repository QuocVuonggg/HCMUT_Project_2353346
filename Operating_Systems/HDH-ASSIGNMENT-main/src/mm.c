...
{                                                   // no guarantee all given pages are mapped
  ...
  ret_rg->rg_end = (pgnum * PAGING_PAGESZ) + addr;
...
addr_t alloc_pages_range(struct pcb_t *caller, int req_pgnum, struct framephy_struct **frm_lst)
{
  ...
  *frm_lst = NULL;
  ...
  for (pgit = 0; pgit < req_pgnum; pgit++){
    ...
    // allocate from RAM first
    if (MEMPHY_get_freefp(caller->krnl->mram, &fpn) == 0){
      newnode->fpn = fpn;
    }
    // if RAM full -> allocate to SWAP
    else if (MEMPHY_get_freefp(caller->krnl->active_mswp, &fpn) == 0){
      newnode->fpn = fpn;
    }
    // out of memory
    else {

...
module RegFile (
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            // Only write when we=1 and not to register 0
            regs[rd] <= rd_data;
        end
    end
...
module DatapathSingleCycle (
    ...
    // Output 4 bit for LED Extension Board to debug CLA
    ...
    // 1. RegFile Connection Signals
    reg reg_we;
    wire [`REG_SIZE:0] rs1_data;
    wire [`REG_SIZE:0] rs2_data;
    reg [`REG_SIZE:0] wb_data; // Write back data
    ...
    reg                alu_cin;       // Carry in
    reg                alu_inv_b;     // Invert B
    ...
    always @(*) begin
        ...
        case (inst_opcode)
            ...
            OpBranch: begin
                ...
                alu_op_b  = rs2_data;
                alu_inv_b = 1'b1; 
                alu_cin   = 1'b1; // +1 => Subtraction rs1 - rs2
            ...
            OpRegReg: begin
                ...
                if (inst_funct7 == 7'd1) begin // M-Extension
                      case (inst_funct3)
                        ...
                        3'b100: begin
                            if (rs2_data == 32'd0)
                                wb_data = 32'hFFFFFFFF; // Division by zero returns -1
                            else if (rs1_data == 32'h80000000 && rs2_data == 32'hFFFFFFFF)
                                wb_data = 32'h80000000; // Overflow
                            else
                                wb_data = $signed(rs1_data) / $signed(rs2_data);
                        end

                        3'b101: wb_data = div_quo; // DIVU (HW Divider)

                        // REM (Signed)
                        3'b110: begin
                            if (rs2_data == 32'd0)
                                wb_data = rs1_data; // Remainder by zero returns dividend
                            else if (rs1_data == 32'h80000000 && rs2_data == 32'hFFFFFFFF)
                                wb_data = 32'd0;    // Overflow
                            else
                                wb_data = $signed(rs1_data) % $signed(rs2_data);
                        end

                        3'b111: wb_data = div_rem; // REMU (HW Divider)
                        
                        default: wb_data = 32'd0;
                     endcase
                end else begin // Base Integer
                    case (inst_funct3)
...
module MemorySingleCycle #(
  ...
  input       [        3:0] store_we_to_dmem
    ...
    initial begin
        $readmemh("D:/Subject/SoCs/Lab/Lab3/mem_initial_contents.hex", mem_array);
    end
